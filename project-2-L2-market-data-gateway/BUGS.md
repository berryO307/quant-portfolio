...
- `/fix`: Operational, not a code fix — capture a real Hyperliquid session (`./build/quant_day1.exe <minutes> <symbol>`), start a relay instance on that instrument's assigned port (`PORT=<port> npx tsx src/index.ts`), and replay the capture into it with `relay/scripts/replay-gateway.mjs <session>.ndjson.gz <port> <cpu_ghz> --loop`. Initially done for just the default BTC instrument (port 8080); the same "I selected X and got nothing" report recurred for ETH, which is what surfaced that every OTHER dropdown entry had the identical gap — done for all 7 (BTC 8080, ETH 8081, SOL 8082, XRP 8083, HYPE 8084, WTI 8085, Brent 8086) as a batch, each verified individually via a direct WS client check against `/live` (real bid/ask, correct level counts, no timeouts). A real-time gateway→relay push client (the "(not yet built)" one `bybitIngestClient.ts` already anticipates) remains unbuilt — worth its own dedicated feature if genuinely live (not replayed) data ever becomes a requirement, since today's entire pipeline, for every instrument, is capture-then-replay, and each of these 7 background processes will need restarting after any machine restart or once its short capture stops feeling "fresh."
